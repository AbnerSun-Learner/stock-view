...
function getAdviceMeta(score: number): AdviceMeta {
  ...
  if (score < 0.8) return { label: "相关性偏高", tagClass: "fair" };
  ...
  return (
    <div className="correlation-card p-6 md:p-8 space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <StatWithTip
          label="走势同向性"
          tooltip="所选时间窗口内，根据两只 ETF 日收益率计算的同向程度（0–1，越高表示涨跌方向越一致）。该指标在综合相关性中约占一半权重。"
          value={fmt(data.scoreA)}
        />
        <StatWithTip
          label="成分重叠"
          tooltip="依据最新披露持仓，按重仓股权重计算的底层重叠程度（0–1，越高表示持仓股票越相似）。该指标在综合相关性中约占一半权重。"
          value={fmt(data.scoreB)}
        />
        <StatWithTip
          label="皮尔逊相关系数"
          tooltip="在同一时间窗口内，两只 ETF 日收益率之间的线性相关程度，取值在 -1 到 1 之间；接近 1 表示同涨同跌的倾向更强。"
          value={data.pearson.toFixed(3)}
        />
        <StatWithTip
          label="样本天数"
          tooltip="参与走势相关性与净值曲线计算的有效交易日数量（两基金均有行情的重叠样本）。"
      ...
      <div className="flex items-center gap-1 mb-1">
        ...
        <span className="group relative shrink-0">
          <HelpCircle
            ...
            strokeWidth={1.5}
          />
          <span className="absolute left-0 top-full mt-2 hidden group-hover:block z-[99999] max-w-[14rem] p-2 text-xs bg-[var(--foreground)] text-[var(--page-bg)] whitespace-normal font-normal tracking-normal pointer-events-none normal-case shadow-sm border border-[color:var(--border-color)]">
            {tooltip}
          </span>
        </span>
      </div>
