...
export function ValuationWeightChart({ holdings }: ValuationWeightChartProps) {
  ...
  const holdingData = useMemo(() => {
    return holdings.slice(0, 8).map((h) => ({ name: h.name, value: h.weight }));
  }, [holdings]);
      ...
      tooltip: {
        ...
        formatter: (params: unknown) => {
          ...
          return `<div style="font-weight:600">${
            p.name
          }</div><div style="font-size:12px;color:#71717a">${p.value.toFixed(
            2
          )}% (占比 ${p.percent.toFixed(1)}%)</div>`;
        },
      },
      series: [
        {
    ...
    () => ({
      tooltip: {
        ...
        formatter: (params: unknown) => {
          ...
          return `<div style="font-weight:600">${
            p.name
          }</div><div style="font-size:12px;color:#71717a">权重 ${p.value.toFixed(
            2
          )}%</div>`;
        },
      },
