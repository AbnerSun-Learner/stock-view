 ...
 * 2. 计算成分重叠
 * 3. 合成综合分与建议
 * 4. 从对齐的收益率序列推导时序图表数据（散点、业绩走势、滚动相关系数）
 */
...
function buildHeadline(score: number): string {
  ...
  if (score < 0.8) return "对比重合度高，重复风险显著";
  return "高度重复，建议精简持仓";
}
...
export function buildPairResponse(
  ...
  if (pairResult.status !== "complete") {
    if (pairResult.status === "unavailable") {
      throw new PairAnalysisUserError(
        pairResult.missingReason ?? "数据不足，无法完成指数对比"
      );
    }
    throw new PairAnalysisUserError(
      `分析数据不完整（${
        pairResult.missingReason ?? "部分数据源不可用"
      }）。「业绩走势」「历史联动趋势」「综合对比分」需在行情与重仓数据均就绪时方能展示；请稍后重试或排查网络与该品种是否可被 TuShare 拉取。`
    );
  }
  ...
  // 业绩走势：序列含日复利累计与当日收益；前端业绩图仅绘制当日估算涨跌幅
  ...
  // 滚动相关系数：60 日窗口 Pearson，直接传索引避免重复 slice
