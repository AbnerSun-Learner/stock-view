...
- **执行记录（续）**：阶段 1.6 已核对——`2026-05-08-etf-correlation-tool-plan.md` 已指向 `doc/superpowers/notes/correlation-validation.md`；`.gitignore` 已补充 `artifacts/`（与规格 §2 本地产物约定一致）；README 已简述 P2 产物目录。
- **执行记录（批 C，2026-05-09）**：维护者确认删除 `.claude/skills/`；`python-utils/` 保留。已更新 `AGENTS.md`、README、本规格与盘点笔记；验证：`pnpm run lint`、`pnpm run build`。
...
| 1.6  | 核对 ETF 相关性计划中对验证笔记文件名的引用：须与 `doc/superpowers/notes/correlation-validation.md` 一致。**已完成**。                                                                                                                                                                       |
...
| 步骤 | 操作                                                                                                                                                                                          |
| ---- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.1  | 在 `.gitignore` 增加本地产物目录，例如：`playwright-artifacts/`、`artifacts/`、`/correlation-v2-light.png` 或更广的 `*-light.png`（按团队是否需保留少量演示图再收窄规则）。原则：P2 不进库。  |
...
**进度（2026-05-09）**：3.1 **`python-utils/` 保留**（维护者确认）；3.2 盘点已完成；`design-system/` 状态见盘点笔记。
...
**进度（2026-05-09）**：4.1–4.2 **已完成**——`.claude/skills/` 目录已从仓库删除；`AGENTS.md` 已改为说明 OpenSkills / Cursor agent_skills；`pnpm run lint` 与 `pnpm run build` 已通过。
