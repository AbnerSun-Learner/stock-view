...
- **未引用** `python-utils/`、`design-system/`、`.claude/skills/` 路径。
...
| `design-system/` | ~16K | P1 | `PAGE_AUDIT.md`、`UI_TEMPLATE.md`；与 `.cursor/skills/stillwell-ui` 职责部分重叠 |
...
## 待你确认后的「可删 / 可迁」候选（请勿在未回复前自动执行）

每项若同意删除或迁出，请明确回复 **编号 + 动作（删除仓库内目录 / 仅本地保留 / 迁出到子模块等）**。

1. **`python-utils/`**  
   - 无 CI 引用；若你不再使用 stillwell-tracker / m1 对比脚本，可考虑删除或迁出单独仓库。
2. **`design-system/`**（两个 Markdown）  
   - [doc/code-review-2026-04-30.md](../../code-review-2026-04-30.md) 曾建议并入 `doc/design-system/`；**移动**也属结构变更，建议与删除一样先确认。
3. **`.claude/skills/`**  
